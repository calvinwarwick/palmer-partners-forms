      ...
      {applicants.map((applicant, index) => <Card key={applicant.id} className="border-2 border-gray-200 bg-gradient-to-br from-white to-orange-50/30 ">
          <CardHeader className="pb-4 bg-orange-500 text-white rounded-t-lg">
            <CardTitle className="text-lg font-semibold flex items-center gap-3 text-white">
              ...
              Applicant {index + 1}
              {applicant.firstName && applicant.lastName && <span className="text-white font-normal">
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                <div className="space-y-2">
                  ...
                  <Input id={`currentAddress-${applicant.id}`} value={applicant.currentAddress || ""} onChange={e => onUpdateApplicant(applicant.id, "currentAddress", e.target.value)} placeholder="Enter your street address" className="form-control border-gray-200 focus:border-orange-500 focus:ring-orange-500 shadow-sm" required />
                </div>
                <div className="space-y-2">
                  <Label htmlFor={`currentPostcode-${applicant.id}`} className="form-label text-gray-700 font-medium">
                    Postcode <span className="text-red-500">*</span>
                  </Label>
                  <Input id={`currentPostcode-${applicant.id}`} value={applicant.currentPostcode || ""} onChange={e => onUpdateApplicant(applicant.id, "currentPostcode", e.target.value)} placeholder="Enter postcode" className="form-control border-gray-200 focus:border-orange-500 focus:ring-orange-500 shadow-sm" required />
                </div>
                <div className="space-y-2">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                <div className="space-y-2">
                  ...
                  <div className="date-input-container">
                    ...
                    <Input id={`moveInDate-${applicant.id}`} type="date" value={applicant.moveInDate || ""} onChange={e => onUpdateApplicant(applicant.id, "moveInDate", e.target.value)} className="form-control border-gray-200 focus:border-orange-500 focus:ring-orange-500 shadow-sm text-left pl-12" required />
                  </div>
                </div>
                <div className="space-y-2">
                  ...
                  <div className="date-input-container">
                    <Calendar className="date-input-icon" />
                    <Input id={`vacateDate-${applicant.id}`} type="date" value={applicant.vacateDate || ""} onChange={e => onUpdateApplicant(applicant.id, "vacateDate", e.target.value)} className="form-control border-gray-200 focus:border-orange-500 focus:ring-orange-500 shadow-sm text-left pl-12" required />
                  </div>
                </div>
                {shouldShowRentalAmount(applicant.currentPropertyStatus || "") && <div className="space-y-2">
                    ...
                    <div className="currency-input-container">
                      ...
                      <Input id={`currentRentalAmount-${applicant.id}`} type="number" value={applicant.currentRentalAmount || ""} onChange={e => onUpdateApplicant(applicant.id, "currentRentalAmount", e.target.value)} placeholder="Enter monthly rental amount" className="currency-input border-gray-200 focus:border-orange-500 focus:ring-orange-500 shadow-sm" required />
                    </div>
                  </div>}
              </div>
            </div>
